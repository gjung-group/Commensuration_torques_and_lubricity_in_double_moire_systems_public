...
!   double precision, parameter :: aG2 = aG,       aBN02 = 2.46019d0 ! extep
   ...
   !t2BN- Circular Flake
   double precision :: phi12deg  =  1.538d0 !1.5d0 ! Substrate : Rotation angle in degrees
   double precision :: phi32deg  =  XXXd0 !1.5d0 ! Flake      : Rotation angle in degrees
   double precision :: Radius    =  40d0 !20d0 ! Flake Radius in aG
!!! G/G
...
! BN/BN
   ...
   !character(*), parameter :: Sp_L1(2) = (/'B','N'/), Sp_L2(2) =(/'B','N'/), Sp_L3(2) =(/'B','N'/) ! L2 is Reference, Middle Layer
...
!   integer, parameter :: N_L1(2) = (/5,7/), N_L2(2) = (/5,7/), N_L3(2) = (/5,7/) ! L2 is Reference,Bottom Layer
   ...
   integer, parameter :: supkFinal = 4 !300 ! Even number 
...
!      write(*,'(a,3i9)') 'FixA: ', 1+ncell(1)*2+ncell(2)*2 !, 1+ncell(1)*2+ncell(2)*2
      write(*,'(a,i9,a,i9,a,i9,a,i9,a,i9)') 'FixA: ', 1,',', ncell(1)*2,',', ncell(1)*2+1,&
                                           &',',ncell(1)*2 +ncell(2)*2 ,',', 1+ncell(1)*2+ncell(2)*2
...
!   open(u4,FILE="BLBL.basis2",STATUS='replace')
   ...
   write(*,'(a)') 'ucell : reference layer lattice vectors, which has the same orientation of Rcell'
   write(*,'(3f16.9)') ucell(:,1)
   write(*,'(3f16.9)') ucell(:,2)
   write(*,'(3f16.9)') ucell(:,3)

   write(*,'(3(a,f16.9))') 'FixB(ucell): '    ,1.0d0,',', ucell(1,1)+ucell(1,2)         ,',', ucell(2,1)+ucell(2,2) 
   write(*,'(3(a,f16.9))') 'FixC(ucell/210): ',1.0d0,',',(ucell(1,1)+ucell(1,2))/210.0d0,',',(ucell(2,1)+ucell(2,2))/210.0d0 !
   write(*,'(3(a,f16.9))') 'FixD(ucell/30): ' ,1.0d0,',',(ucell(1,1)+ucell(1,2))/30.0d0 ,',',(ucell(2,1)+ucell(2,2))/30.0d0  !
